...
var Vim = function() {
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (range.empty()) {
        if (range.anchor.ch < cm.getLine(range.anchor.line).length) {
          result.push(
            cm.markText(range.anchor, Pos(range.anchor.line, range.anchor.ch + 1), { className: 'cm-fat-cursor-mark' }),
          )
        } else {
  ...
  var motions = {
    ...
    goToMark: function(cm, _head, motionArgs, vim) {
      ...
      if (pos) {
        return motionArgs.linewise ? { line: pos.line, ch: cm.findFirstNonWhiteSpaceCharacter(pos.line) } : pos
      }
      return null
    },
  ...
  function doReplace(cm, confirm, global, lineStart, lineEnd, searchCursor, query, replaceWith, callback) {
    ...
    function replaceAll() {
      ...
      cm.operation(
        function(matches, matchesLength) {
